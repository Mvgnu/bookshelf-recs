...
## 2024-05-28
- Added `/api/communities/search` endpoint to allow filtering communities by name.
- Extended OpenAPI spec, API docs and README with search details.
- Updated `CommunityManager` React component with search box and inline editing for owners.
- Created `frontend/src/components/README.md` documenting UI components.

## 2024-05-29
- Added structured musikconnect tags at the top of `backend/app.py` for machine-readable metadata.
- Created `backend/README.md` summarizing backend purpose and usage.
- Installed Python dependencies so tests run successfully.
- Documented remaining ESLint issue due to missing `@eslint/js` package.

## 2024-05-30
- Inserted musikconnect metadata headers into all React components and `App.jsx`.
- Noted the metadata approach in `frontend/src/components/README.md`.
- Linting still fails offline due to missing `@eslint/js` packages.

## 2024-05-31
- Implemented `/api/token/refresh` and `/api/logout` for JWT lifecycle management.
- Revoked tokens are stored in-memory and checked by `token_required`.
- Updated API reference and README with usage instructions.
- Added integration tests covering token refresh and logout behavior.

## 2024-06-01
- Replaced deprecated `Query.get` calls with `db.session.get` throughout the backend to silence SQLAlchemy warnings.
- All tests pass after dependency installation.

## 2024-06-02
- Added `scripts/install_offline_node.sh` to install frontend packages from local tarballs for offline linting.
- Updated README with instructions on using the script.
- Created `scripts/` directory to host utility scripts.

## 2024-06-03
- Investigated splitting `backend/app.py` into Blueprints for maintainability.
- Initial attempt caused circular imports when importing models into the blueprint module.
- Reverted to monolithic structure for now and documented the plan to modularize later.
