"""
# musikconnect tags
purpose: Flask API server providing book analysis, user management, and community features
inputs: HTTP requests, uploaded images
outputs: JSON responses, static frontend files
status: active
depends_on: Flask, SQLAlchemy, Flask-Limiter, requests-cache, google-generativeai
related_docs: README.md, docs/API_REFERENCE.md
"""
...
# Track revoked JWTs so they cannot be used after logout
REVOKED_TOKENS = set()
...
OPENAPI_SPEC = {
    ...
    "paths": {
        ...
        "/api/token/refresh": {"post": {"summary": "Refresh an auth token"}},
        "/api/logout": {"post": {"summary": "Invalidate the current token"}},
        "/api/bookshelves": {
        ...
        "/api/communities/search": {"get": {"summary": "Search communities"}},
...
def logout_user():
    ...
    logger.info(f"Token revoked for user_id: {g.user_id}")
...
def list_friends():
    ...
    for fr in friendships:
        ...
        user = db.session.get(User, friend_id)
        if user:
...
def list_friend_requests():
    ...
    for fr in requests_q:
        requester = db.session.get(User, fr.requester_id)
        if requester:
...
def list_outgoing_requests():
    ...
    results = []
    for fr in pending:
        user = db.session.get(User, fr.addressee_id)
        if user:
...
@app.route('/api/communities/search')
def search_communities():
    """Search communities by name."""
    query = request.args.get('q', '').strip()
    if not query:
        return jsonify([]), 200
    communities = Community.query.filter(Community.name.ilike(f"%{query}%"))\
...
def join_community(comm_id):
    ...
    community = db.session.get(Community, comm_id)
    if not community:
